...
app = FastAPI(
    title="API de Predicción de Granizo - Nimbus AI",
    version="3.1.0",
    docs_url="/api/docs",            # Swagger UI accesible en /api/docs
    redoc_url="/api/redoc",          # Redoc accesible en /api/redoc
    openapi_url="/api/openapi.json"  # OpenAPI JSON accesible en /api/openapi.json
)

# Permitir CORS (ajustar allow_origins en producción)
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
# La ruta absoluta al modelo dentro del contenedor
MODEL_PATH = "/code/models/multimodal_v3.1_optimizado.keras"
# La ruta absoluta al scaler dentro del contenedor
SCALER_PATH = "/code/data/processed/modelo_multimodal/sets_finales/scaler.pkl"
...
def procesar_imagen_api(ruta_archivo_nc):
    ...
    # Escalar coordenadas internas según metadatos
    ds['x'] = ds['x'] * h_sat
    ds['y'] = ds['y'] * h_sat
    # Selección espacial y variable CMI (Canal 13 esperado en L2 CMIPF)
    ...
    # Normalizar entre 0 y 1 (evitar divisiones por cero)
...
@app.get("/api/health")
def health():
    return {"status": "ok", "service": "Nimbus-AI", "version": "3.1.0"}

# --- 5. El Endpoint de Predicción Definitivo (ahora en /api/predict) ---
...
def predict_hail(forecast_input: ForecastInput):
    ...
    try:
        ...
        # C. Asegurar que todas las columnas esperadas existan (según el scaler)
        ...
        # D. Lógica de Imagen (con fallback a archivo local de muestra)
        ...
        try:
            ...
            # Fallback a imagen de muestra incluida en el repo
        ...
        # Si tu modelo espera múltiples canales, replicamos la imagen a lo largo de canales
