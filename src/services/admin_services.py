...
def suspend_user_service(user_id):
    ...
    try:
        ...
        app_logger.info(f"Usuario {user.email} (ID: {user_id}) ha sido suspendido exitosamente.")
        return {"success": True, "msg": f"Usuario {user.email} ha sido suspendido."}
    except Exception as e:
...
def unban_user_service(user_id):
    ...
    try:
        ...
        app_logger.info(f"La suspensión del usuario {user.email} (ID: {user_id}) ha sido levantada.")
        return {"success": True, "msg": f"La suspensión de {user.email} ha sido levantada."}
    except Exception as e:
